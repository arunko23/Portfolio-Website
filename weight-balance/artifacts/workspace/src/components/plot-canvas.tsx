    ...
    <div className="flex min-w-0 flex-col gap-3">
      <div className="no-print flex flex-wrap items-center justify-between gap-2">
        ...
        <p className="text-xs text-muted">
          Overlay only — each CG follows its own printed millimetre line from the
          bottom tick to the top tick (slopes are not the same).
        </p>
      </div>
        ...
        >
          ...
          >
            ...
            <g clipPath="url(#plot-clip)">
              ...
              {showCgLine
                ? [...new Map(points.map((p) => [p.cgMm.toFixed(3), p])).values()].map((p) => (
                    <CgConstruction key={`cg-${p.id}`} cgMm={p.cgMm} />
                  ))
                : null}
...
function CgConstruction({ cgMm }: { cgMm: number }) {
  const clipped = cgLineClipped(cgMm);
