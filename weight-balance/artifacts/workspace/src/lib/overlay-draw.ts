...
function uniqueCgValues(points: PlottedPoint[]) {
  const seen = new Set<string>();
  const out: number[] = [];
  for (const p of points) {
    const k = p.cgMm.toFixed(3);
...
function drawCgConstruction(ctx: CanvasRenderingContext2D, cgMm: number) {
  const line = cgLineClipped(cgMm);
  if (!line) return;
...
export function drawOverlays(ctx: CanvasRenderingContext2D, state: OverlayState) {
  ...
  if (showCgLine) {
    for (const cgMm of uniqueCgValues(points)) {
      drawCgConstruction(ctx, cgMm);
    }
    for (const p of points) {
  ...
  if (readout) {
    const line = cgLineClipped(readout.cgMm);
    if (line) {
      ctx.save();
      ctx.setLineDash([4, 4]);
      haloStroke(
...
export async function rasterizeChart(
  ...
  canvas.height = Math.round((CHART.imageHeight + extra) * pixelRatio);
  const ctx = canvas.getContext("2d");
  if (!ctx) throw new Error("Could not create print canvas.");
  ctx.fillStyle = "#ffffff";
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  ctx.drawImage(image, 0, 0, canvas.width, CHART.imageHeight * pixelRatio);
