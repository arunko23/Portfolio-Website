...
/** Slope (dx/dy) of the printed 12 / 14 / 16 … 40 family at the top of the graph. */
export const INDEX_DX_DY = -0.571;

/**
 * After the weight × CG intersection, continue UP-RIGHT parallel to the
 * numbered 12–40 lines (x increases as y decreases).
 */
export function indexRayFromPoint(pt: ChartPoint): { start: ChartPoint; end: ChartPoint } | null {
  // Stop at the top of the plotted graph (1000 kg line), not in the margin.
  const yTop = CHART.plotTop;
  const xAtTop = pt.x + INDEX_DX_DY * (yTop - pt.y);
  ...
  return { start: clipped.bottom, end: clipped.top };
}

/** Millimetre CG line from the bottom tick up to the plotted point only. */
export function cgLineToPoint(cgMm: number, pt: ChartPoint) {
  const raw = cgLine(cgMm);
  return clipSegment(
    raw.bottom,
    pt,
